# Week 6 — Agentic Development with Warp

Use the app in `week6/` as your playground. This week mirrors the prior assignment but emphasizes the Warp agentic development environment and multi‑agent workflows.
...
2) Run the app (from `week6/` directory)
...
### Testing
Run the tests (from `week6/` directory)
```bash
make test
```
...
Work strictly in `week6/` (backend, frontend, logic, tests). Avoid changing other weeks unless the automation explicitly requires it and you document why.
...
2) A write‑up `writeup.md` under `week6/` that includes:
   - Which tasks you implemented
